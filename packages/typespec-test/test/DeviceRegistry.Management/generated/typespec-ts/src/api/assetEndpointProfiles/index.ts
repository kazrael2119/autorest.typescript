  ...
  return {
    ...
    properties: !res.body.properties
      ...
      : {
          ...
          targetAddress: res.body.properties?.["targetAddress"],
          userAuthentication: !res.body.properties?.userAuthentication
            ? undefined
            : {
                mode: res.body.properties?.userAuthentication?.["mode"],
                usernamePasswordCredentials: !res.body.properties
                  ?.userAuthentication?.usernamePasswordCredentials
                  ? undefined
                  : {
                      usernameReference:
                        res.body.properties?.userAuthentication
                          ?.usernamePasswordCredentials?.["usernameReference"],
                      passwordReference:
                        res.body.properties?.userAuthentication
                          ?.usernamePasswordCredentials?.["passwordReference"],
                    },
                x509Credentials: !res.body.properties?.userAuthentication
                  ?.x509Credentials
                  ? undefined
                  : {
                      certificateReference:
          ...
          transportAuthentication: !res.body.properties?.transportAuthentication
            ? undefined
            : {
                ownCertificates: res.body.properties?.transportAuthentication?.[
                  "ownCertificates"
                ].map((p) => {
                  return {
                    certThumbprint: p["certThumbprint"],
          ...
          additionalConfiguration:
            res.body.properties?.["additionalConfiguration"],
...
export async function _updateDeserialize(
  ...
  const res = result as unknown as AssetEndpointProfilesUpdateLogicalResponse;
  return {
    tags: res.body["tags"],
    location: res.body["location"],
    id: res.body["id"],
